...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func setUpData() {
        GeneralLocationList.sharedInstance.addNewGeneralLocation(name:"Phandalin",type:"City",desc:"The town consisted of forty or fifty simple log buildings, some build on old fieldstone foundations. More old ruins - crumbling stone walls covered in ivy and briars - surround the newer buildings.", image:"asset_phandalin")
        var location1: GeneralLocation = GeneralLocationList.sharedInstance.locations[0]
        location1.addNewLocation(name:"Stonehill Inn", type:"Inn", address:"Somewhere in Phandalin", desc:"Toblen came from the east of the Triboar, seeking the opportunity in prospecting like many others. He soon found that he knew more about running an inn than mining, and so he established the inn.", image:"asset_inn")
        var character1 = Character(name: "Aram Silverheart", species: "Human", location: "Aram's Traders", desc: "An old human, bent and bowed. He carries a walking stick in his left hand, but something suggests he might use it for more than walking", notes: "Gruff voice. Likes carpentry.", image:"asset_human_male")
        CharacterList.sharedInstance.addCharacterToList(char: character1)
        GeneralLocationList.sharedInstance.locations[0].addNewLocation(name:"The Sleeping Giant", type:"Tavern", address:"Somewhere in Phandalin", desc:"An old crumbling stone building with boarded up windows.",image:"asset_inn")
